...
//編集部分
	...
	$filename2="mission_3-4-13.txt";
        ...
        // 書き込むデータを作る
        $writeData = ($_POST["edit_post"] ?: count($lines) + 1) . "<>" . $_POST["name"] . "<>" . $_POST["comment"]."<>".$_POST["Password3"];

        // 編集番号があればデータループして場所を特定して上書きする
	
        if(isset($_POST["edit_post"])&&isset($_POST["Password3"])) {
            // データ件数分処理(&で参照にしてる)
            foreach($lines as $row) {
                // <>で分割して配列に
                $Data = explode("<>", $row);
		echo $Data[0]."<br>";
		echo $_POST["Password3"]."<br>";
		echo var_export($_POST["Password3"] == $Data[4]);
                // 編集番号のところだったら上書き
                if($Data[0] == $_POST["edit_post"]) {
			echo $Data[0]."aaa<br>";
			if($bData[4]===$_POST["Password3"]){
                    		$row = $writeData;
			}
                }
            }
